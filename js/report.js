/**
 * report.js (Excel Style Clean Version)
 */
  ...
  function _renderBillDetails(rows) {
    ...
    rows.forEach((row, i) => {
      ...
      tr.innerHTML = `
        ...
        <td class="p-2 border border-black">${_col(row, "SPARK_CODE") || "—"}</td>
        <td class="p-2 border border-black text-center">${_col(row, "ENCASH_DATE") || "—"}</td>
        <td class="p-2 border border-black text-right font-bold text-black">${_fmt(_colNum(row, "GROSS_AMOUNT"))}</td>
        <td class="p-2 border border-black text-right">${_fmt(_colNum(row, "PAY"))}</td>
        <td class="p-2 border border-black text-right">${_fmt(_colNum(row, "DA"))}</td>
        <td class="p-2 border border-black text-right">${_fmt(_colNum(row, "HRA"))}</td>
        <td class="p-2 border border-black text-right">${_fmt(_colNum(row, "CCA"))}</td>
        <td class="p-2 border border-black text-right">${_fmt(_colNum(row, "PG_ALLOWANCE"))}</td>
        <td class="p-2 border border-black text-right">${_fmt(_colNum(row, "RURAL_ALLOWANCE"))}</td>
        <td class="p-2 border border-black text-right">${_fmt(_colNum(row, "OTHER_ALLOWANCE"))}</td>
        <td class="p-2 border border-black text-right">${_fmt(_colNum(row, "CONSOLIDATE_PAY"))}</td>
        <td class="p-2 border border-black text-right">${_fmt(_colNum(row, "DAILY_WAGES"))}</td>
        <td class="p-2 border border-black text-right">${_fmt(_colNum(row, "MS"))}</td>
        <td class="p-2 border border-black text-right">${_fmt(_colNum(row, "TOUR_TA"))}</td>
        <td class="p-2 border border-black text-right">${_fmt(_colNum(row, "MR"))}</td>
        <td class="p-2 border border-black">${_col(row, "REMARKS") || "—"}</td>
      `;
    ...
    trTotal.innerHTML = `
      <td class="p-2 border border-black text-right uppercase" colspan="3">Grand Total</td>
      <td class="p-2 border border-black text-right">₹ ${_fmt(totals.grossAmount)}</td>
      <td class="p-2 border border-black text-right">${_fmt(totals.pay)}</td>
      <td class="p-2 border border-black text-right">${_fmt(totals.da)}</td>
      <td class="p-2 border border-black text-right">${_fmt(totals.hra)}</td>
      <td class="p-2 border border-black text-right">${_fmt(totals.cca)}</td>
      <td class="p-2 border border-black text-right">${_fmt(totals.pgAllowance)}</td>
      <td class="p-2 border border-black text-right">${_fmt(totals.ruralAllowance)}</td>
      <td class="p-2 border border-black text-right">${_fmt(totals.otherAllowance)}</td>
      <td class="p-2 border border-black text-right">${_fmt(totals.consolidatePay)}</td>
      <td class="p-2 border border-black text-right">${_fmt(totals.dailyWages)}</td>
      <td class="p-2 border border-black text-right">${_fmt(totals.ms)}</td>
      <td class="p-2 border border-black text-right">${_fmt(totals.tourTa)}</td>
      <td class="p-2 border border-black text-right">${_fmt(totals.mr)}</td>
