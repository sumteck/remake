/**
 * report.js (Excel Style - ID Bug Fixed, Auto Previous Month & Progressive Totals)
 * =====================================
 */
...
const TbrReport = (() => {
  ...
  // ID പ്രശ്നം പരിഹരിച്ച ഭാഗം (രണ്ട് പേരുകളും കോഡ് തനിയെ നോക്കും)
  ...
  function _renderBillDetails(currentRows, prevTotals) {
    ...
    keys.forEach(k => { 
      htmlPrev += `<td class="p-1 border border-black text-right outline-none focus:bg-yellow-100 prev-month-cell" contenteditable="true" data-key="${k}" title="Click to edit previous month amount">${_fmt(prevTotals[k])}</td>`; 
    });
    ...
    keys.forEach(k => { 
      const progTotal = totals[k] + prevTotals[k];
      htmlProg += `<td class="p-1 border border-black text-right" id="prog-${k}">${_fmt(progTotal)}</td>`; 
    });
  ...
  function _renderReportHeader(finYear, month, rows) {
    ...
    const dateEl = $("report-month-year") || $("print-month-year");
    if (dateEl) dateEl.textContent = `${month} ${finYear}`;
    
    if (rows && rows.length > 0) {
      const trsyEl = $("report-treasury") || $("print-treasury");
      ...
      if ($("print-treasury")) $("print-treasury").textContent = "";
  ...
  async function _generateReport() {
    ...
    try {
      ...
      const headerRefRow = currentRows.length > 0 ? currentRows : prevRows;
      
      _renderReportHeader(finYear, month, headerRefRow);
      _renderBillDetails(currentRows, prevTotals);
      ...
      else toast(`No data found for ${month}, but showing previous totals.`, "info");
