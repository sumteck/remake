...
const TbrDashboard = (() => {
  ...
  // 👇 ഗൂഗിൾ ഷീറ്റിലെ കോമയോ ചിഹ്നങ്ങളോ കണ്ട് തെറ്റാതിരിക്കാൻ ചേർത്ത പുതിയ ഫംഗ്ഷൻ 
  function _parseSheetAmount(val) {
  ...
  async function _loadPeriodData() {
    ...
    try {
      ...
      _tableRows = sheetRows.map(row => ({
        ...
        // ഇവിടെ പുതിയ ഫംഗ്ഷൻ ഉപയോഗിക്കുന്നു
        basicLess: _parseSheetAmount(row[C.BASIC]), 
        da: _parseSheetAmount(row[C.DA]), 
        hra: _parseSheetAmount(row[C.HRA]), 
        cca: _parseSheetAmount(row[C.CCA]),
        otherAllowances: _parseSheetAmount(row[C.OTHER_ALLOWANCES]), 
        netAmount: _parseSheetAmount(row[C.NET_AMOUNT]),
    ...
    filteredRows.forEach((row) => {
      ...
      COL_DEFS.forEach(col => {
        ...
        if (col.key === "billNo") {
          ...
          td.className += " tracking-wide text-sky-900"; 
      ...
      btn.textContent = "✕"; 
      btn.addEventListener("click", () => _deleteRow(row));
