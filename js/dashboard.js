...
const TbrDashboard = (() => {
  ...
  async function _loadPeriodData() {
    ...
    try {
      ...
      _tableRows = sheetRows.map(row => ({
        ...
        ddoCode: row[C.DDO_CODE] || "", department: row[C.DEPT] || "", 
  ...
  function _renderTable() {
    ...
    if (_tableRows.length === 0) { if (emptyState) show(emptyState); return; }
    if (emptyState) hide(emptyState);

    _tableRows.forEach((row, idx) => {
      const tr = document.createElement("tr"); tr.className = "border-b border-gray-200 hover:bg-sky-50"; tr.dataset.idx = idx;
      COL_DEFS.forEach(col => {
        const td = document.createElement("td"); td.className = "px-2 py-2 whitespace-nowrap text-gray-700";
        
        // ടേബിളിൽ വെച്ച് നേരിട്ട് എഡിറ്റ് ചെയ്യാവുന്ന ഇൻപുട്ട് ബോക്സുകൾ
        if (col.key === "encashmentDate") {
          ...
          inp.addEventListener("change", (e) => { _tableRows[idx][col.key] = e.target.value; });
          td.appendChild(inp);
        } else if (col.key === "remarks") {
          const inp = document.createElement("input");
          inp.type = "text";
          inp.className = "border border-sky-300 rounded px-2 py-1 text-xs w-[160px] outline-none focus:ring-2 focus:ring-sky-500 bg-white";
          inp.value = row[col.key] || "";
          inp.addEventListener("input", (e) => { _tableRows[idx][col.key] = e.target.value; });
          td.appendChild(inp);
        } else if (col.key === "netAmount") {
          td.className += " text-right font-mono font-bold text-sky-700";
      ...
      btn.textContent = "✕"; btn.addEventListener("click", () => _deleteRow(idx)); tdDel.appendChild(btn); tr.appendChild(tdDel);
      tbody.appendChild(tr);
    });
    const badge = $("row-count-badge"); if (badge) badge.textContent = `${_tableRows.length} row${_tableRows.length !== 1 ? "s" : ""}`;
  }

  function _deleteRow(idx) { _tableRows.splice(idx, 1); _renderTable(); }
  ...
  function _bindManualEntryForm() {
    ...
    addBtn.addEventListener("click", async () => {
      ...
      const row = { billType, billNo, sparkCode, treasury, ddoCode, department, basicLess, da, hra, cca, otherAllowances, netAmount, encashmentDate, remarks, rawHoA: hoaStr, canonicalHoA: TbrParser.canonicalHoA(hoa), ...hoa };
  ...
  function _bindClearButton() {
    ...
    btn.addEventListener("click", () => {
      ...
      if (confirm("Clear all rows from the preview table?")) { _tableRows = []; _renderTable(); toast("Table cleared.", "info"); }
    });
  }
  ...
  function _onSignOut() { _tableRows = []; _renderTable(); toast("Signed out.", "info"); }

  function init() {
    _populatePeriodSelectors(); _bindPeriodSelectors(); _bindPdfUpload(); _bindManualEntryForm(); _bindSaveButton(); _bindClearButton(); _renderTable();
