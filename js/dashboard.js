/**
 * dashboard.js
 * Manages UI interactions, PDF uploads, and Google Sheets integration.
 */
...
const TbrApp = (() => {
  ...
  function _formatBillsForSheet(bills) {
    return bills.map(b => [
      b.billType || "", b.billNo || "", b.treasury || "", b.headOfAccount || "",
      b.sparkCode || "", b.department || "", b.pay || 0, b.da || 0,
      b.hra || 0, b.cca || 0, b.pgAllowance || 0, b.ruralAllowance || 0,
      b.otherAllowance || 0, b.consolidatePay || 0, b.dailyWages || 0, b.ms || 0,
      b.tourTa || 0, b.mr || 0, b.grossAmount || 0, b.encashDate || "", b.remarks || ""
    ]);
  }
  ...
  // കൃത്യമായി ഒറ്റ വരി മാത്രം ഷീറ്റിൽ നിന്നും ഡിലീറ്റ് ചെയ്യുന്നു
  function removeRow(index) {
    const billToDelete = parsedBills[index];
    ...
    showConfirmModal(
      ...
      async () => {
        ...
        if (TbrAuth.isSignedIn()) {
          ...
          if (finYear && month) {
            setLoading(true, "Deleting from Google Sheets...");
            try {
              // api.js ലെ പുതിയ deleteSingleBill ഫംഗ്ഷൻ വിളിക്കുന്നു
              await TbrApi.deleteSingleBill(finYear, month, billToDelete.sparkCode, billToDelete.grossAmount);
              toast("Bill successfully deleted from sheet!", "success");
            } catch (err) {
  ...
  async function handlePdfUpload(files) {
    ...
    for (const file of files) {
      ...
      try {
        ...
        results.forEach(res => {
          ...
          const newSparkCode = String(res.sparkCode || "").trim();
          const newGross = String(res.grossAmount || "").trim();

          const isDuplicate = parsedBills.some(b => {
            const oldSparkCode = String(b.sparkCode || "").trim();
            const oldGross = String(b.grossAmount || "").trim();
          ...
          if (isDuplicate) {
            ...
            parsedBills.unshift({
              ...res,
              headOfAccount: res.rawHoA || "", 
              encashDate: res.encashDate || "",
            }); 
            successCount++; 
          }
        });
    ...
    setLoading(false);
    if (duplicateCount > 0) toast(`${duplicateCount} duplicate bill(s) skipped.`, "info");
    if (successCount > 0) {
      renderTable();
      toast(`Added ${successCount} new bill(s) to preview.`, "success");
    }
  }
