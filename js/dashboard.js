...
const TbrDashboard = (() => {
  ...
  const COL_DEFS = [
    ...
    { key: "netAmount",   label: "Gross Amount"   },
  ];
    ...
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        const sheetRows = _tableRows.map(r => {
          ...
          // പൂജ്യം നിലനിർത്താനുള്ള മാറ്റങ്ങൾ ഇവിടെ ചേർത്തിരിക്കുന്നു
          row[C.MJH        - 2] = r.MJH ? `'${r.MJH}` : "'00";
          row[C.SMJH       - 2] = r.SMJH ? `'${r.SMJH}` : "'00";
          row[C.MIH        - 2] = r.MIH ? `'${r.MIH}` : "'00";
          row[C.SBHLH      - 2] = r.SBHLH ? `'${r.SBHLH}` : "'00";
          row[C.SHLH       - 2] = r.SHLH ? `'${r.SHLH}` : "'00";
          row[C.VOH        - 2] = r.VOH ? `'${r.VOH}` : "'00";
