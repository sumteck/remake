    ...
    for (let line of lines) {
      if (/^Total\s+\d+/i.test(line.trim())) {
        ...
        if (nums.length >= 9) {
          ...
          result.da             = nums[2] || 0; // DA
          result.hra            = nums[3] || 0; // HRA
          result.cca            = nums[4] || 0; // CCA
          result.pgAllowance    = nums[5] || 0; // PGA
          result.ruralAllowance = nums[6] || 0; // RL_AL
          
          // താങ്കൾ പറഞ്ഞ ലോഗിക്: Rural Allowance-ന് ശേഷമുള്ളവ ആഡ് ചെയ്ത് Other Allowance-ൽ ആക്കുന്നു
          let knownSum = result.pay + result.da + result.hra + result.cca + result.pgAllowance + result.ruralAllowance;
          let otherAllw = 0;
          let i = 7; // എട്ടാമത്തെ സംഖ്യ (Sp. L.Sal) മുതൽ തുടങ്ങുന്നു
          
          while (i < nums.length) {
            // Gross Salary ആണോ എന്ന് ചെക്ക് ചെയ്യുന്നു
            if (nums[i] === (knownSum + otherAllw) && nums[i] !== 0) {
              result.grossAmount = nums[i];
              break;
            }
            // Gross Salary അല്ലെങ്കിൽ അത് Other Allowance ലേക്ക് കൂട്ടുന്നു (Sp. L.Sal ഉൾപ്പെടെ)
            otherAllw += nums[i];
            i++;
          }
          ...
          if (result.grossAmount === 0) {
            result.otherAllowance = nums[7] || 0;
            result.grossAmount    = nums[8] || 0;
          } else {
