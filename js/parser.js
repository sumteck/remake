...
const TbrParser = (() => {
  ...
  // ── പുതിയത്: Period കണ്ടുപിടിച്ച് Remarks തയ്യാറാക്കുന്നു ──
  function _extractPeriodRemarks(text) {
    let m = text.match(/Received\s*for\s*the\s*Period\s*[:\s]*([0-9\/\-\s]+)/i);
    ...
    m = text.match(/Salary\s*for\s*([a-zA-Z]+\s*[0-9]{4})/i);
    if (m) return "Salary for " + m[1].trim();
  ...
  function _extractSparkCodeString(text) {
    const m = text.match(/Spark\s*Code\s*:\s*([0-9\s]{15,})/i);
    return m ? m[1].trim() : "";
  }
  ...
  function _extractSparkOfficeAndDept(text) {
    ...
    if (dept && office && !office.includes(dept)) return `${dept} - ${office}`;
    return office || dept;
  }

  function _extractSparkAllowances(text) {
    let basicLess = 0, da = 0, hra = 0, cca = 0, grossSalary = 0;
    const bMatch = text.match(/\b(?:01|140)\s+Pay.*?\s+([0-9,]+(?:\.\d{1,2})?)/i);
    if (bMatch) basicLess = parseAmount(bMatch[1]);
    const dMatch = text.match(/\b(?:22|141)\s+DA.*?\s+([0-9,]+(?:\.\d{1,2})?)/i);
    if (dMatch) da = parseAmount(dMatch[1]);
    const hraMatch = text.match(/\bHRA[^\d]*?([0-9,]+(?:\.\d{1,2})?)/i);
    if (hraMatch) hra = parseAmount(hraMatch[1]);
    const ccaMatch = text.match(/\bCCA[^\d]*?([0-9,]+(?:\.\d{1,2})?)/i);
    if (ccaMatch) cca = parseAmount(ccaMatch[1]);
    const grossMatch = text.match(/Total\s*A\s*Gross\s*[:=]?\s*([0-9,]+(?:\.\d{1,2})?)/i) || text.match(/Gross\s*Salary[\s\S]*?([0-9,]+(?:\.\d{1,2})?)/i);
  ...
  function parseSparkBill(text) {
    ...
    const result = {
      ...
      ddoCode: "", department: _extractSparkOfficeAndDept(text),
      ...
      remarks: _extractPeriodRemarks(text), // Remarks ഓട്ടോമാറ്റിക് ആയി വരും
      encashmentDate: "", // ഇത് മാനുവൽ ആയി ടേബിളിൽ അടിക്കാം
      hoa: null,
    };
  ...
  function parseBimsBill(text) {
    const result = {
      ...
      sparkCode: "N/A", treasury: "", ddoCode: "", department: "",
    ...
    if (mDept) result.department = mDept[1].replace(/Bill\s*No.*$/i, '').trim();
  ...
  async function parsePdf(file) {
    ...
    const baseRow = {
      ...
      treasury: parsed.treasury, ddoCode: parsed.ddoCode, department: parsed.department,
