...
const TbrParser = (() => {
 ...
 function _extractSparkCodeString(text) {
  ...
  function _extractSparkAllowances(text) {
    ...
    // സൂപ്പർ ട്രിക്ക്: അലവൻസിന്റെ പേരിന് ശേഷം വരുന്ന കൃത്യമായ കറൻസി തുക (അതായത് അവസാനം .00 ഉള്ളത്) മാത്രം വലിച്ചെടുക്കുന്നു
    function findAmount(keyword) {
      // [^A-Za-z]{0,40}? എന്നത് ഇടയിലുള്ള സ്പേസുകളെയോ അക്കങ്ങളെയോ മാത്രം കടത്തിവിട്ട് യഥാർത്ഥ തുകയെ കണ്ടെത്തും
      const regex = new RegExp(keyword + "[^A-Za-z]{0,40}?([0-9]{1,3}(?:,[0-9]{2,3})*\\.\\d{2}|[0-9]+\\.\\d{2})", "i");
      const match = text.match(regex);
      return match ? parseAmount(match[1]) : 0;
    }

    // SPARK കോഡുകൾ (01, 22, 123 ഒക്കെ) ഉണ്ടെങ്കിലും ഇല്ലെങ്കിലും പേര് വെച്ച് കൃത്യമായി തുക എടുക്കും
    basicLess = findAmount("\\b(?:\\d{2,3}\\s+)?Pay\\b");
    da = findAmount("\\b(?:\\d{2,3}\\s+)?DA\\b");
    hra = findAmount("\\b(?:\\d{2,3}\\s+)?(?:HRA|House\\s*Rent(?:\\s*Allowance)?)\\b");
    cca = findAmount("\\b(?:\\d{2,3}\\s+)?CCA\\b");
