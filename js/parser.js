...
const TbrParser = (() => {
   ...
   */
// =========================================================================
  // STAGE 4 — ULTIMATE HYBRID SALARY ENGINE (GLUED-TEXT & PAGE 2 FIXED)
  // =========================================================================
  ...
  function _extractSalaryBreakdown(fullText) {
    ...
    // 1. Gross Salary (Page 1 Abstract-ൽ നിന്നും)
    const mGross = fullText.match(/(?:Total\s*A\s*Gross|Gross\s*Salary)[\s:\-]*([\d,]+(?:\.\d+)?)/i);
    if (mGross) result.grossAmount = parseAmount(mGross[1]);

    // 2. Commonly used Dues ടേബിൾ ഒഴിവാക്കാൻ Page 1 മാത്രം മുറിച്ചെടുക്കുന്നു (ക്ലോഡിന് പറ്റിയ അബദ്ധം ഒഴിവാക്കാൻ)
    const abstractText = fullText.split(/Commonly\s*used\s*Dues/i)[0];

    // 3. Glued-Text Proof Regex: വാക്കുകൾ സ്പേസ് ഇല്ലാതെ ഒട്ടിപ്പിടിച്ചാലും (ഉദാ: 01Pay73600) കൃത്യമായി തുക എടുക്കും!
    const extractCodeValue = (code) => {
      // (?<!\d) = വേറെ നമ്പറിന്റെ കൂടെയല്ല എന്ന് ഉറപ്പാക്കുന്നു. 
      // (?:[^\d]|\(\d+\))*? = ഇടയിലുള്ള അക്ഷരങ്ങളും (64) പോലുള്ള ബ്രാക്കറ്റുകളും സ്കിപ്പ് ചെയ്ത് നേരെ തുക എടുക്കുന്നു.
      const regex = new RegExp(`(?<!\\d)${code}(?:[^\\d]|\\(\\d+\\))*?([\\d,]+)`, 'i');
      const m = abstractText.match(regex);
    ...
    // അഥവാ കിട്ടിയില്ലെങ്കിൽ Page 2-ലെ Total വരിയിൽ നിന്നും എടുക്കുന്നു (ഇവിടെ fullText തന്നെ ഉപയോഗിക്കണം)
    if (result.pay === 0) {
      ...
      while ((match = tableRegex.exec(flatText)) !== null) {
        ...
        // ഗ്രോസ് സാലറി ആ വരിയിലുണ്ടെങ്കിൽ അത് ശരിയായ സാലറി വരിയാണെന്ന് ഉറപ്പിക്കുന്നു
    ...
    // 5. Other Allowance = Gross - (Pay + Allowances)
