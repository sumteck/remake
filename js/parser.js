...
const TbrParser = (() => {
  ...
  // =========================================================================
  // STAGE 4 — EXPLICIT PAGE 2 SALARY EXTRACTION ENGINE (FIXED)
  // =========================================================================
  ...
  function _extractSalaryBreakdown(fullText) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // 'Total' എന്ന് തുടങ്ങുന്ന വരി കണ്ടെത്തുന്നു
      if (line.toLowerCase().includes("total")) {
        let nums = line.match(/\d+/g);
        
        // അഥവാ PDF-ൽ 'Total' എന്ന വാക്കും അക്കങ്ങളും രണ്ട് വരിയിലായി മുറിഞ്ഞു പോയാൽ ഉള്ള ഫിക്സ്
        ...
        // സാലറി ടേബിളിലെ 9 കോളങ്ങൾ കൃത്യമായി മാപ്പ് ചെയ്യുന്നു
        if (nums && nums.length >= 9) {
          const parsedNums = nums.map(Number);
          
          result.pay            = parsedNums[1] || 0; // Basic Less OA/SA
          result.da             = parsedNums[2] || 0; // DA
          result.hra            = parsedNums[3] || 0; // HRA
          result.cca            = parsedNums[4] || 0; // CCA
          result.pgAllowance    = parsedNums[5] || 0; // PG Allowance (Dashboard: PG Allowance)
          result.ruralAllowance = parsedNums[6] || 0; // Rural Allowance (Dashboard: Rural Allowance)
          
          // താങ്കൾ പറഞ്ഞ ലോഗിക്: Rural Allowance (Index 6) കഴിഞ്ഞു വരുന്ന എല്ലാ അലവൻസുകളും കൂട്ടുന്നു
          let knownSum = result.pay + result.da + result.hra + result.cca + result.pgAllowance + result.ruralAllowance;
          let otherSum = 0;
          let grossAmt = 0;
          
          // Index 7 (Sp. L.Sal) മുതൽ മുന്നോട്ട് ഗ്രോസ് സാലറി എത്തുന്നതുവരെ കൂട്ടുന്നു
          ...
          // മാത്തമാറ്റിക്കൽ മാച്ചിംഗ് വിജയിച്ചാൽ അത് എടുക്കുന്നു, ഇല്ലെങ്കിൽ ഫിക്സഡ് ഇൻഡെക്സ് ബാക്കപ്പ് ഉപയോഗിക്കുന്നു
          if (grossAmt > 0) {
