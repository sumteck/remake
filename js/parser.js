...
const TbrParser = (() => {
  ...
  // =========================================================================
  // STAGE 4 — EXPLICIT PAGE 2 SALARY EXTRACTION ENGINE (FALSE-ROW FILTERED)
  // =========================================================================
  ...
  function _extractSalaryBreakdown(fullText) {
    ...
    // ഫസ്റ്റ് പാസ്: മാത്തമാറ്റിക്കൽ മാച്ചിങ് പക്കാ ആയി വിജയിക്കുന്ന പേജ് 2-ലെ വരി മാത്രം തിരയുന്നു
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.toLowerCase().includes("total")) {
        ...
        // അഥവാ വരി മുറിഞ്ഞുപോയിട്ടുണ്ടെങ്കിൽ തൊട്ടടുത്ത വരി കൂടി നോക്കുന്നു
        if (!nums || nums.length < 9) {
          let nextLine = (lines[i + 1] || "").trim();
          let nextNums = nextLine.match(/\d+/g);
          if (nextNums && nextNums.length >= 9) {
            nums = nextNums;
          }
        }
        ...
        if (nums && nums.length >= 9) {
          ...
          const pay            = parsedNums[1] || 0; // Basic Less OA/SA
          const da             = parsedNums[2] || 0; // DA
          const hra            = parsedNums[3] || 0; // HRA
          const cca            = parsedNums[4] || 0; // CCA
          const pgAllowance    = parsedNums[5] || 0; // PG Allowance
          const ruralAllowance = parsedNums[6] || 0; // Rural Allowance
          
          let knownSum = pay + da + hra + cca + pgAllowance + ruralAllowance;
          let otherSum = 0;
          let grossAmt = 0;
          
          // Rural Allowance കഴിഞ്ഞുള്ളവ കൂട്ടി ഗ്രോസ് സാലറിയുമായി ഒത്തുനോക്കുന്നു
          for (let j = 7; j < parsedNums.length; j++) {
            if (parsedNums[j] === (knownSum + otherSum) && parsedNums[j] > 0) {
              grossAmt = parsedNums[j];
              break;
            }
            otherSum += parsedNums[j];
          }
          
          // കണക്ക് കൃത്യമായി മാച്ച് ആയാൽ മാത്രം ഇത് പേജ് 2-ലെ സാലറി വരിയാണെന്ന് ഉറപ്പിച്ച് ഡാറ്റ എടുക്കുന്നു
          if (grossAmt > 0) {
            result.pay            = pay;
            result.da             = da;
            result.hra            = hra;
            result.cca            = cca;
            result.pgAllowance    = pgAllowance;
            result.ruralAllowance = ruralAllowance;
            result.otherAllowance = otherSum;
            result.grossAmount    = grossAmt;
            return result; // കറക്റ്റ് വരി കിട്ടിയാൽ ഉടൻ ഫംഗ്ഷൻ നിർത്തുന്നു
          }
        }
      }
    }

    // സെക്കൻഡ് പാസ് (Fallback): അഥവാ മുകളിലെ മാച്ചിങ് നടന്നില്ലെങ്കിൽ മാത്രം ആദ്യത്തെ ടോട്ടൽ വരി ബാക്കപ്പായി എടുക്കുന്നു
    for (let i = 0; i < lines.length; i++) {
      let line = lines[i].trim();
      if (line.toLowerCase().includes("total")) {
        let nums = line.match(/\d+/g);
        if (nums && nums.length >= 9) {
          const parsedNums = nums.map(Number);
          result.pay            = parsedNums[1] || 0;
          result.da             = parsedNums[2] || 0;
          result.hra            = parsedNums[3] || 0;
          result.cca            = parsedNums[4] || 0;
          result.pgAllowance    = parsedNums[5] || 0;
          result.ruralAllowance = parsedNums[6] || 0;
          result.otherAllowance = parsedNums[7] || 0;
          result.grossAmount    = parsedNums[8] || 0;
          return result;
        }
      }
    }
