...
const TbrParser = (() => {
   ...
   */
// =========================================================================
  // STAGE 4 — ULTIMATE HYBRID SALARY ENGINE (ISOLATED ABSTRACT FIX)
  // =========================================================================
  ...
  function _extractSalaryBreakdown(fullText) {
    ...
    // 2. THE FIX: ഡേറ്റും Head of Account-ഉം വന്ന് തെറ്റാതിരിക്കാൻ "ABSTRACT OF THE BILL" സെക്ഷൻ മാത്രം മുറിച്ചെടുക്കുന്നു!
    ...
    if (absMatch) {
      ...
      abstractText = fullText.split(/Commonly\s*used\s*Dues/i)[0]; // Fallback
    }

    // 3. Glued-Text Proof Regex (വാക്കുകൾ ഒട്ടിപ്പിടിച്ചാലും വർക്ക് ആകും)
    const extractCodeValue = (code) => {
      // മുറിച്ചെടുത്ത Abstract സെക്ഷനിൽ മാത്രം കോഡ് സ്കാൻ ചെയ്യുന്നു
      const regex = new RegExp(`\\b${code}\\b(?:[^\\d]|\\(\\d+\\))*?([\\d,]+)`, 'i');
