...
const TbrParser = (() => {
  ...
  function _extractSparkAllowances(text) {
    ...
    const grossMatch = text.match(/Total\s*A\s*Gross\s*[:=]?\s*([0-9,]+(?:\.\d{1,2})?)/i) || 
                       text.match(/Gross\s*Salary[^\n\r]*?([0-9,]+(?:\.\d{1,2})?)/i);
    if (grossMatch) grossSalary = parseAmount(grossMatch[1]);

    // പേരിന്റെ തൊട്ടടുത്തുള്ള വരിയിൽ നിന്നും മാത്രം തുക എടുക്കുന്നു. [^\n\r]*? എന്നത് ആ വരിക്ക് പുറത്തേക്ക് പോകാൻ അനുവദിക്കില്ല.
    const bMatch = text.match(/\b(?:01|140)\s*[-]?\s*Pay[^\n\r]*?([0-9,]+\.\d{2})/i);
    if (bMatch) basicLess = parseAmount(bMatch[1]);

    const dMatch = text.match(/\b(?:22|141)\s*[-]?\s*DA[^\n\r]*?([0-9,]+\.\d{2})/i);
    if (dMatch) da = parseAmount(dMatch[1]);

    const hMatch = text.match(/\b(?:123\s*)?[-]?\s*(?:HRA|House\s*Rent)[^\n\r]*?([0-9,]+\.\d{2})/i);
    if (hMatch) hra = parseAmount(hMatch[1]);

    const cMatch = text.match(/\b(?:24\s*)?[-]?\s*CCA[^\n\r]*?([0-9,]+\.\d{2})/i);
    if (cMatch) cca = parseAmount(cMatch[1]);

    // അഥവാ മുകളിലത്തേത് ഒന്നും കിട്ടിയില്ലെങ്കിൽ ഏറ്റവും സുരക്ഷിതമായ Total വരിയിൽ നിന്നും എടുക്കും (Fallback)
    if (basicLess === 0 && da === 0 && hra === 0 && cca === 0) {
