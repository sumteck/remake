...
const TbrParser = (() => {
  ...
  function _extractSalaryBreakdown(fullText) {
    ...
    // വരികൾ മുറിഞ്ഞുപോകുന്നത് തടയാൻ മുഴുവൻ ടെക്സ്റ്റും ഒറ്റ വരിയാക്കുന്നു
    const flatText = fullText.replace(/\n/g, " ");

    // 'Total' കഴിഞ്ഞു വരുന്ന അക്കങ്ങളുടെ ശ്രേണി കണ്ടെത്തുന്നു
    ...
    while ((match = regex.exec(flatText)) !== null) {
      ...
      if (nums.length >= 8) {
        ...
        // മാത്തമാറ്റിക്കൽ ക്രോസ് ചെക്കിംഗ് (Arrear bill-ൽ 0 വന്നാലും വർക്ക് ആകും)
        const knownSum = pay + da + hra + cca + pgAllowance + ruralAllowance;
        let otherAllw = 0;
        let i = 7; // Sp. L.Sal മുതൽ മുന്നോട്ട് കൂട്ടുന്നു
    ...
    // ഫുൾ ബാക്കപ്പ് ലോഗിക്: ഒരു കീവേർഡും ഇല്ലെങ്കിലും സംഖ്യകളുടെ കൂട്ടം വെച്ച് കണ്ടെത്തുന്നു
