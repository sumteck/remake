...
const TbrParser = (() => {
 ...
 function _extractSparkCodeString(text) {
  ...
  function _extractSparkAllowances(text) {
    ...
    // തുകകൾ എടുക്കുന്നത് കൂടുതൽ കൃത്യമാക്കിയിരിക്കുന്നു
    const bMatch = text.match(/\b(?:01|140)\s+Pay[^\d]{0,40}?([0-9,]+(?:\.\d{1,2})?)/i);
    if (bMatch) basicLess = parseAmount(bMatch[1]);
    
    const dMatch = text.match(/\b(?:22|141)\s+DA[^\d]{0,40}?([0-9,]+(?:\.\d{1,2})?)/i);
    if (dMatch) da = parseAmount(dMatch[1]);
    
    // HRA അല്ലെങ്കിൽ House Rent എന്ന് കണ്ടാലും എടുക്കും
    const hraMatch = text.match(/\b(?:123\s+)?(?:HRA|House\s*Rent)[^\d]{0,40}?([0-9,]+(?:\.\d{1,2})?)/i);
    if (hraMatch) hra = parseAmount(hraMatch[1]);
    
    const ccaMatch = text.match(/\b(?:24\s+)?CCA[^\d]{0,40}?([0-9,]+(?:\.\d{1,2})?)/i);
