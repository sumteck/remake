...
const TbrParser = (() => {
  ...
  // =========================================================================
  // STAGE 4 — BULLETPROOF SALARY EXTRACTION ENGINE (UNIVERSAL ARREAR FIX)
  // =========================================================================
  ...
  function _extractSalaryBreakdown(fullText) {
    ...
    // വരികൾ തമ്മിലുള്ള പ്രശ്നം ഒഴിവാക്കാൻ മുഴുവൻ ടെക്സ്റ്റും ഒറ്റ വരിയിലാക്കുന്നു (Flattening)
    const flatText = fullText.replace(/\n/g, " ");

    // 'Total' എന്ന വാക്കിന് ശേഷം വരുന്ന അക്കങ്ങളുടെ കൂട്ടം കണ്ടെത്തുന്നു
    const regex = /\bTotal\b\s+((?:\d+\s+){7,}\d+)/ig;
    let match;
    let foundNums = [];
    ...
    while ((match = regex.exec(flatText)) !== null) {
      ...
      if (nums.length >= 8) {
        const pay            = nums[1] || 0; // Basic Less OA/SA
        const da             = nums[2] || 0; 
        const hra            = nums[3] || 0; 
        const cca            = nums[4] || 0; 
        const pgAllowance    = nums[5] || 0; 
        const ruralAllowance = nums[6] || 0; 
        
        // മാത്തമാറ്റിക്കൽ ക്രോസ് ചെക്കിംഗ്: അലവൻസുകൾ കൂട്ടിയ തുക ഗ്രോസ് സാലറിയുമായി മാച്ച് ആകുന്നുണ്ടോ എന്ന് നോക്കുന്നു
        const knownSum = pay + da + hra + cca + pgAllowance + ruralAllowance;
        let otherAllw = 0;
        let i = 7; // Sp. L.Sal (Index 7) മുതൽ സ്കാൻ ചെയ്യുന്നു
        let isCorrectRow = false;
        let tempGross = 0;
        let tempOther = 0;

        while (i < nums.length) {
          // തുകകൾ തമ്മിൽ കൂട്ടി നോക്കുമ്പോൾ Gross Salary ഫിഗർ കൃത്യമായി മാച്ച് ആയാൽ ഇത് തന്നെയാണ് സാലറി വരിയെന്ന് ഉറപ്പിക്കാം
          if (nums[i] === (knownSum + otherAllw) && nums[i] > 0) {
            tempGross = nums[i];
            tempOther = otherAllw;
            isCorrectRow = true;
            break;
        ...
        if (isCorrectRow) {
          ...
          result.otherAllowance = tempOther;
          result.grossAmount    = tempGross;
          return result; // കൃത്യമായ വരി കണ്ടെത്തി കഴിഞ്ഞാൽ ഇവിടെ വെച്ച് നിർത്തുന്നു
        }
      }
    }

    // ഫുൾ ബാക്കപ്പ്: അഥവാ 'Total' എന്ന കീവേർഡ് മാച്ച് ആയില്ലെങ്കിൽ ഏത് നീളമുള്ള സംഖ്യാശ്രേണിയും ഇതേ ലോഗിക് വെച്ച് ചെക്ക് ചെയ്യുന്നു
    const anySeqRegex = /((?:\d+\s+){8,}\d+)/g;
    while ((match = anySeqRegex.exec(flatText)) !== null) {
      const nums = match[1].trim().split(/\s+/).map(Number);
      if (nums.length >= 9) {
        const pay            = nums[1] || 0;
        const da             = nums[2] || 0;
