...
// =========================================================================
  ...
  function _extractBimsDetails(text) {
    ...
    // 1. BRN (Bill Reference Number) - കൃത്യമായി 20 അക്കങ്ങൾ എടുക്കുന്നു
    const brnMatch = text.match(/(?<!\d)(\d{20})(?!\d)/);
    if (brnMatch) {
      result.brn = brnMatch[1].trim();
    }

    // 2. The Magic Colon Sequence Logic:
    // PDF-ൽ ലേബലുകൾ വേർപെട്ടു പോകുന്നതുകൊണ്ട്, 10 അക്കങ്ങളുള്ള DDO കോഡിനെ അടിസ്ഥാനമാക്കി 
    // അതിനുമുന്നിലും പിന്നിലുമുള്ള കോളനുകളുടെ (:) ഓർഡർ വെച്ച് ഡാറ്റ കൃത്യമായി മുറിച്ചെടുക്കുന്നു.
    if (result.brn) {
      // ഓർഡർ: : (Date) : (Nature) : (Treasury) : (Department) : (10-digit DDO) : (Office ... up to BRN)
      const blockRegex = /:\s*([^:]+?)\s*:\s*([^:]+?)\s*:\s*([^:]+?)\s*:\s*([^:]+?)\s*:\s*(\d{10})\s*:\s*([\s\S]+?)(?=\b\d{20}\b)/i;
      const match = text.match(blockRegex);
      
      if (match) {
        result.treasury   = match[3].replace(/\n/g, " ").trim();
        result.department = match[4].replace(/\n/g, " ").trim();
        result.ddoCode    = match[5].trim();
        
        // Office-ന്റെ പേര് രണ്ട് വരിയിലായി വന്നാലും കൃത്യമായി എടുക്കാൻ
        let officeName = match[6].replace(/\n/g, " ").trim();
        // അനാവശ്യ വാക്കുകൾ വന്നിട്ടുണ്ടെങ്കിൽ ഒഴിവാക്കുന്നു
        officeName = officeName.replace(/Period\s*of\s*claim/i, "").trim();
        result.office = officeName;
      }
    }
