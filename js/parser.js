...
// =========================================================================
  ...
  function _extractBimsDetails(text) {
    ...
    // 1. BRN നമ്പർ എടുക്കുന്നു
    const brnMatch = text.match(/(?<!\d)(\d{20})(?!\d)/);
    if (brnMatch) {
      result.brn = brnMatch[1].trim();
    }

    // 2. Treasury മാത്രം എടുക്കാൻ ഏറ്റവും കൃത്യമായ ലോഗിക്: 
    // ടെക്സ്റ്റിലെ കോളനുകൾക്കിടയിൽ 'Treasury' എന്ന വാക്ക് വരുന്ന ഭാഗം മാത്രം തിരഞ്ഞുപിടിച്ച് എടുക്കുന്നു!
    const flatText = text.replace(/\n/g, " ");
    const trMatch = flatText.match(/:\s*([^:]*?Treasury[^:]*?)(?=\s*:)/i);
    
    if (trMatch) {
      result.treasury = trMatch[1].trim(); // ഉദാ: Sub Treasury, Perinthalmanna എന്ന് കൃത്യമായി കിട്ടും
    }
