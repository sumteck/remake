...
const TbrParser = (() => {
      ...
      for (const y of sortedY) {
        ...
        for (const item of lineItems) {
          ...
          if (prevRight !== -1) {
            ...
            // നമ്പറുകൾക്കിടയിലെ ഗ്യാപ്പ് ചെറുതാണെങ്കിൽ മാത്രം ഒന്നിപ്പിക്കുന്നു (eg: 736 00 -> 73600)
            if (gap < 5 && /^[\d,]+$/.test(prevStr.trim()) && /^[\d,]+$/.test(item.str.trim())) {
              lineStr += item.str;
            } 
            // അക്ഷരങ്ങൾക്കിടയിലെ തീരെ ചെറിയ ഗ്യാപ്പ് ആണെങ്കിൽ മാത്രം ഒന്നിപ്പിക്കുക (pdf rendering fix)
            else if (gap < 1.5) {
              lineStr += item.str;
            }
            // അല്ലാത്തവയ്ക്ക് കൃത്യമായി സ്പേസ് കൊടുക്കുക
            else {
              lineStr += " " + item.str;
            }
          } else {
  ...
  function _extractTreasury(text) {
    // 'Name Of Treasury:' മുതൽ ആ വരിയുടെ അവസാനം (\n) വരെ മാത്രം എടുക്കുന്നു
    const m = text.match(/Name\s+Of\s+Treasury\s*[:\-]?\s*([^\n\r]+)/i);
    
    if (m) {
      // അഥവാ ഒരേ വരിയിൽ വേറെ അനാവശ്യ ലേബലുകൾ വന്നാൽ അവിടെ വെച്ച് കട്ട് ചെയ്യുന്നു
      let treasuryName = m[1].split(/(?:Computer|Token|Scroll|Dept|DDO|Date)/i)[0];
