...
const TbrParser = (() => {
 ...
 function _extractSparkNetAmount(text) {
    // SPARK-ലെ 'Total A Gross:' അല്ലെങ്കിൽ 'Gross Salary' കണ്ടെത്താൻ
    let m = text.match(/Total\s*A\s*Gross:\s*([0-9,]+(?:\.[0-9]{1,2})?)/i);
  ...
  function _extractBimsNetAmount(text) {
    // BiMS-ലെ 'Gross Bill Amount Rs.' കണ്ടെത്താൻ
    let m = text.match(/Gross\s*Bill\s*Amount\s*Rs\.?\s*([0-9,]+(?:\.[0-9]{1,2})?)/i);
    if (m) return parseAmount(m[1]);
    return 0;
  }
