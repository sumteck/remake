...
const TbrParser = (() => {
  ...
  function _extractTreasury(text) {
    // അക്ഷരങ്ങൾ ഒട്ടിപ്പോയാലും കണ്ടുപിടിക്കാനും, അനാവശ്യ വാക്കുകൾ ഒഴിവാക്കാനുമുള്ള പവർഫുൾ ലോഗിക്
    const m = text.match(/(?:Name\s*Of\s*Treasury|Treasury)\s*[:\-]?\s*(.+?)(?=\s{2,}|\n|\r|\bComputer\b|\bToken\b|\bScroll\b|\bDept\b|\bDDO\b|\bDate\b|$)/i);
    
    if (m) {
      let treasuryName = m[1];
      // അഥവാ എന്തെങ്കിലും അനാവശ്യ വാക്കുകൾ ബാക്കി വന്നാൽ അതിനെ ഒന്നുകൂടി ക്ലീൻ ചെയ്യുന്നു
      treasuryName = treasuryName.split(/(?:Computer|Token|Scroll|Dept|DDO|Date)/i)[0];
