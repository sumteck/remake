...
const TbrParser = (() => {
  ...
  function _extractTreasury(text) {
    // വരകൾ (___), കുത്തുകൾ (....), സ്ലാഷ് (/) എന്നിവ ഒഴിവാക്കാൻ അക്ഷരങ്ങളും അക്കങ്ങളും ബ്രാക്കറ്റും മാത്രം എടുക്കുന്നു.
    const m = text.match(/(?:Name\s*Of\s*Treasury|Treasury)\s*[:\-]?\s*([A-Za-z0-9\s,\(\)]+)/i);
    
    if (m) {
      let tName = m[1];
      
      // അബദ്ധത്തിൽ മറ്റ് ലേബലുകൾ ആ വരിയിൽ ഉണ്ടെങ്കിൽ അത് കട്ട് ചെയ്ത് കളയുന്നു
      tName = tName.split(/(?:Computer|Token|Scroll|Dept|DDO|Date|Form|Vide|Officer)/i)[0];
      tName = tName.trim();
      
      // ചുരുങ്ങിയത് 3 അക്ഷരങ്ങൾ എങ്കിലും ഉണ്ടെങ്കിൽ മാത്രം പേര് കൊടുക്കുന്നു
      if (tName.length > 2) {
        return tName;
      }
    }
