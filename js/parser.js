...
// =========================================================================
 ...
 // =========================================================================
  ...
  function _extractBimsDetails(text) {
    ...
    // 5. Office name (Claude's two-part extraction)
    const officePart1Match = text.match(/Designation of DDO\s*:\s*[\s\S]*?:\s*([\s\S]+?)\s*Name of Office/i);
    const officePart2Match = text.match(/TAN\/GIR\s*:\s*\S+\s+([\s\S]+?)\s*\d*\s*Bill Reference/i);

    const rawPart1 = officePart1Match ? officePart1Match[1].trim() : "";
    const part1 = rawPart1.replace(/^[\s\S]*:\s*/, "").trim(); 
  ...
  async function parseBimsBillFull(file) {
    ...
    return {
      ...
      department:     deptDisplay,        // ഇവിടെ ഡിപ്പാർട്ട്മെന്റും ഓഫീസും ഒന്നിച്ച് വരും!
