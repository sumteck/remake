...
// =========================================================================
 ...
 function _extractBimsDetails(text) {
    ...
    // 1. BRN (Bill Reference Number)
    const brnMatch = text.match(/(?<!\d)(\d{20})(?!\d)/);
    if (brnMatch) {
      result.brn = brnMatch[1].trim();
    }

    // പിഡിഎഫിലെ ടെക്സ്റ്റിനെ ഒറ്റ വരിയായി മാറ്റുന്നു (സ്ക്രീൻഷോട്ടിൽ കണ്ടതുപോലെ)
    const flatText = text.replace(/\n/g, " ");

    // 2. Treasury (Name of Treasury മുതൽ Name of Department വരെയുള്ള ഭാഗം)
    const trMatch = flatText.match(/Name\s+of\s+Treasury\s*:\s*(.*?)\s*Name\s+of\s+Department/i);
    if (trMatch) {
      result.treasury = trMatch[1].trim();
    }

    // 3. Department (Name of Department മുതൽ DDO Code വരെയുള്ള ഭാഗം)
    const deptMatch = flatText.match(/Name\s+of\s+Department\s*:\s*(.*?)\s*DDO\s+Code/i);
    if (deptMatch) {
      result.department = deptMatch[1].trim();
    }

    // 4. DDO Code
    const ddoMatch = flatText.match(/DDO\s+Code\s*:\s*(\d{10})/i);
    if (ddoMatch) {
      result.ddoCode = ddoMatch[1].trim();
    }

    // 5. Office (Name of Office മുതൽ TAN/GIR അല്ലെങ്കിൽ Bill Reference വരെയുള്ള ഭാഗം)
    const offMatch = flatText.match(/Name\s+of\s+Office\s*:\s*(.*?)\s*(?:TAN\/GIR|Bill\s+Reference)/i);
    if (offMatch) {
      result.office = offMatch[1].trim();
    }
