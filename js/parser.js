...
const TbrParser = (() => {
   ...
   */
// =========================================================================
  // STAGE 4 — SMART HYBRID SALARY ENGINE (CLAUDE'S BUG FIXED)
  // =========================================================================
  ...
  function _extractSalaryBreakdown(fullText) {
    ...
    // 1. Gross Salary (Authoritative)
    const mGross = fullText.match(/(?:Total\s*A\s*Gross|Gross\s*Salary)[\s:\-]*([\d,]+(?:\.\d+)?)/i);
    if (mGross) result.grossAmount = parseAmount(mGross[1]);

    // 2. പിഡിഎഫിന്റെ താഴെയുള്ള ഡിക്ഷ്ണറി ടേബിൾ (Commonly used Dues) ഒഴിവാക്കാൻ ടെക്സ്റ്റ് മുറിക്കുന്നു
    const abstractText = fullText.split(/Commonly\s*used\s*Dues/i)[0];

    // 3. ക്ലോഡിന്റെ അബദ്ധം തിരുത്തിയ കൃത്യമായ Regex (ബ്രാക്കറ്റിലെ കോഡുകളെ തുകയായി തെറ്റിദ്ധരിക്കില്ല)
    const extractCodeValue = (code) => {
      const regex = new RegExp(`\\b${code}\\b[\\s\\.\\-]*[A-Za-z\\s\\/\\-]+(?:\\(\\d+\\))?[\\s:]*([\\d,]+)`, 'i');
    ...
    // 4. Pay (Basic Less OA/SA)
    result.pay = extractCodeValue('01');
    
    if (result.pay === 0) {
      const flatText = abstractText.replace(/\n/g, " ");
    ...
    // 5. താങ്കൾ പറഞ്ഞ ലോഗിക്: Other Allowance = Gross - (Pay + Allowances)
