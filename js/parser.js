...
const TbrParser = (() => {
  ...
  function _extractTreasury(text) {
    // 'Name Of Treasury' എന്ന് കൃത്യമായി നോക്കുന്നു (സ്പേസ്, കോളൻ, ഡാഷ് എന്നിവ എങ്ങനെയുണ്ടെങ്കിലും വർക്ക് ആകും)
    const m = text.match(/Name\s*Of\s*Treasury[\s:\-]*([^\n\r]+)/i);
    
    if (m) {
      let tName = m[1];
      
      // ഒരേ വരിയിൽ വരാൻ സാധ്യതയുള്ള മറ്റു ഫീൽഡുകൾ വെച്ച് കൃത്യമായി കട്ട് ചെയ്യുന്നു (Officer ഒഴിവാക്കി)
      tName = tName.split(/(?:Computer\s*Sequence|Token\s*No|Scroll\s*No|Dept\s*Code|DDO\s*Code|Date|Form|Vide)/i)[0];
      
      // അവസാനം ആവശ്യമില്ലാത്ത ചിഹ്നങ്ങളോ (eg: -, :) സ്പേസോ ഉണ്ടെങ്കിൽ അത് റിമൂവ് ചെയ്യുന്നു
      return tName.replace(/[\-:\s]+$/, "").trim();
    }
    return "";
  }
