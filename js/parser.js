...
const TbrParser = (() => {
  ...
  function _extractTreasury(text) {
    // 'Name Of Treasury:' എന്നതിന് ശേഷം ആ വരിയുടെ അവസാനം വരെ എന്തുണ്ടെങ്കിലും അത് കൃത്യമായി എടുക്കുന്നു
    const m = text.match(/Name\s*Of\s*Treasury\s*[:\-]?\s*([^\n\r]+)/i);
    
    if (m) {
      // എടുത്ത പേരിനൊപ്പം അബദ്ധത്തിൽ താഴെയുള്ള വരിയിലെ 'Computer' അല്ലെങ്കിൽ 'Token' പോലുള്ള വാക്കുകൾ വന്നാൽ അവിടെ വെച്ച് കട്ട് ചെയ്യുന്നു
