...
const TbrParser = (() => {
  ...
  // 👇 ഇതിലാണ് 7.09 പോലെയുള്ള അബദ്ധങ്ങൾ ഒഴിവാക്കാനുള്ള കോഡ് ഉള്ളത്!
  function _extractSparkAllowances(text) {
    let basicLess = 0, da = 0, hra = 0, cca = 0, grossSalary = 0;

    const gMatch = text.match(/Total\s*A\s*Gross[\s\S]{1,60}?([0-9]{3,}(?:,[0-9]{2,3})*\.\d{2})/i) || text.match(/Gross\s*Salary[\s\S]{1,60}?([0-9]{3,}(?:,[0-9]{2,3})*\.\d{2})/i);
    if (gMatch) grossSalary = parseAmount(gMatch[1]);

    // പേജ് 3-ലെ "Total" വരിയിൽ നിന്നും നേരിട്ട് എടുക്കുന്നു (ഏറ്റവും സുരക്ഷിതം)
    const totalRowMatch = text.match(/Total\s+([0-9,]+\.\d{2})\s+([0-9,]+\.\d{2})\s+([0-9,]+\.\d{2})\s+([0-9,]+\.\d{2})\s+([0-9,]+\.\d{2})\s+([0-9,]+\.\d{2})/i);
    
    if (totalRowMatch) {
        basicLess   = parseAmount(totalRowMatch[2]);
        da          = parseAmount(totalRowMatch[3]);
        hra         = parseAmount(totalRowMatch[4]);
        cca         = parseAmount(totalRowMatch[5]);
        if(grossSalary === 0) grossSalary = parseAmount(totalRowMatch[6]);
    } else {
        // [0-9]{2,} എന്ന് കൊടുത്തതുകൊണ്ട് 7.09 പോലെയുള്ള ഒറ്റയക്ക ശതമാനങ്ങളെ പൂർണ്ണമായും ഒഴിവാക്കും!
        const bMatch = text.match(/(?:01|140)\s*Pay[\s\S]{1,60}?([0-9]{2,}(?:,[0-9]{2,3})*\.\d{2})/i) || text.match(/\bPay\b[\s\S]{1,40}?([0-9]{2,}(?:,[0-9]{2,3})*\.\d{2})/i);
        if (bMatch) basicLess = parseAmount(bMatch[1]);

        const dMatch = text.match(/(?:22|141)\s*DA[\s\S]{1,60}?([0-9]{2,}(?:,[0-9]{2,3})*\.\d{2})/i) || text.match(/\bDA\b[\s\S]{1,40}?([0-9]{2,}(?:,[0-9]{2,3})*\.\d{2})/i);
        if (dMatch) da = parseAmount(dMatch[1]);

        const hMatch = text.match(/(?:123\s*)?(?:HRA|House\s*Rent)[\s\S]{1,60}?([0-9]+(?:,[0-9]{2,3})*\.\d{2})/i);
        if (hMatch) hra = parseAmount(hMatch[1]);

        const cMatch = text.match(/(?:24\s*)?CCA[\s\S]{1,60}?([0-9]+(?:,[0-9]{2,3})*\.\d{2})/i);
