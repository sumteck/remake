...
const TbrParser = (() => {
 ...
 function _extractSparkAllowances(text) {
    ...
    // 1. Gross Salary (ഏറ്റവും കൃത്യമായി എടുക്കുന്നു)
    let gMatch = text.match(/Total\s*A\s*Gross[^\d]*?([0-9,]+\.\d{2})/i) ||
                 text.match(/Gross\s*Salary[^\d]*?([0-9,]+\.\d{2})/i);
    if (gMatch) grossSalary = parseAmount(gMatch[1]);

    // 2. കൃത്യമായ അലവൻസ് കോഡുകൾ ഉപയോഗിച്ച് മാത്രം തുക എടുക്കുന്നു (01, 22, 123, 24)
    const extractVal = (pattern) => {
        const m = text.match(pattern);
        return m ? parseAmount(m[1]) : 0;
    };

    basicLess = extractVal(/(?:01|140)\s*Pay\D{0,40}?([0-9,]+\.\d{2})/i);
    da = extractVal(/(?:22|141)\s*DA\D{0,40}?([0-9,]+\.\d{2})/i);
    hra = extractVal(/123\s*(?:HRA|House\s*Rent)\D{0,40}?([0-9,]+\.\d{2})/i);
    cca = extractVal(/24\s*CCA\D{0,40}?([0-9,]+\.\d{2})/i);

    // അഥവാ ഡിപ്പാർട്ട്മെന്റ് മാറി കോഡ് ഇല്ലാതെ വെറും പേരുകൾ മാത്രമാണെങ്കിൽ (Fallback)
    if (basicLess === 0) basicLess = extractVal(/\bPay\D{0,20}?([0-9,]+\.\d{2})/i);
    if (da === 0) da = extractVal(/\bDA\D{0,20}?([0-9,]+\.\d{2})/i);
    if (hra === 0) hra = extractVal(/\b(?:HRA|House\s*Rent)\D{0,20}?([0-9,]+\.\d{2})/i);
    if (cca === 0) cca = extractVal(/\bCCA\D{0,20}?([0-9,]+\.\d{2})/i);
