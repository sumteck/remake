...
// =========================================================================
  ...
  // =========================================================================
  // STAGE 7 — BIMS PARSER (PERFECTED FOR PDF.JS COLUMN SPLIT)
  // =========================================================================
  ...
  function _extractBimsDetails(text) {
    ...
    // 2. BiMS കോളൻ ലോഗിക്: 
    if (result.brn) {
      // പാറ്റേൺ: : (Treasury) : (Department) : (10-digit DDO) : (Office ... up to BRN)
      const blockRegex = /:\s*([^:]+?)\s*:\s*([^:]+?)\s*:\s*(\d{10})\s*:\s*([\s\S]+?)(?=\b\d{20}\b)/;
      const match = text.match(blockRegex);
      
      if (match) {
        result.treasury   = match[1].replace(/\n/g, " ").trim();
        result.department = match[2].replace(/\n/g, " ").trim();
        result.ddoCode    = match[3].trim();
        result.office     = match[4].replace(/\n/g, " ").trim();
        return result;
      }
    }
  ...
  async function parseBimsBillFull(file) {
    ...
    return {
      billType:       "BiMS",
      billNo:         "",                 // താങ്കൾ ആവശ്യപ്പെട്ടതുപോലെ Bill No കോളം വെറുതെ വിടുന്നു
      treasury:       bimsData.treasury,  // കൃത്യമായ Treasury പേര്
      sparkCode:      bimsData.brn,       // Spark Code/BRN കോളത്തിൽ BRN നമ്പർ മാത്രം വരും
      department:     deptDisplay,        // Department - Office എന്നിവ ഒരുമിച്ച് വരും
