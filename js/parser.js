...
const TbrParser = (() => {
  ...
  function _extractSalaryBreakdown(fullText) {
    ...
    // =========================================================================
    // LAYER A — GROSS SALARY
    // Locate the authoritative total from the ABSTRACT section.
    // Two label variants are tried in priority order.
    // =========================================================================

    const grossMatch =
      fullText.match(/Total\s*A\s*Gross\s*[\s:\-]*([\d,]+(?:\.\d+)?)/i) ||
      fullText.match(/Gross\s*Salary\s*[\s:\-]*([\d,]+(?:\.\d+)?)/i);
    ...
    // =========================================================================
    // LAYER B — STATUTORY-CODE EXTRACTOR (helper)
    //
    // Format in the Abstract section:
    //   "<code> <label text> [optional (code)] <amount>"
    //
    // Examples the regex must handle (all real variants observed):
    //   "22 DA/ADA 25760"
    //   "23 HRA 4500"
    //   "24 CCA 500"
    //   "64 PG Allowance(64) 1100"
    //   "45 Rural Allowance 800"
    //   "01 Pay/LS/SP/Wages/TP 73600"
    //   "01Pay/LS/SP/Wages/TP73600"   ← pathological merged variant
    //
    // Regex design:
    //   \b<code>\b            — word-boundary-anchored code number
    //   [A-Za-z\s\/\(\)]+     — label: letters, spaces, slashes, parens
    //   (?:\(\d+\))?          — optional trailing parenthetical code repeat
    //   [\s:]*                — optional separator
    //   ([\d,]+)              — CAPTURE: the amount
    //
    // The label segment uses a possessive-style approach: it allows letters,
    // spaces, slashes, and parentheses only — digits are NOT allowed in the
    // middle segment.  This prevents the regex from prematurely consuming
    // the amount digits as part of the label, which was the core failure mode
    // in the original extractCodeValue implementation.
    // =========================================================================

    /**
     * _codeAmount(code, text)
     * Returns the amount associated with a statutory allowance code, or 0.
     * @param {number|string} code  The 2-digit GO code (e.g. 22, 01).
     * @param {string}        text  Full document text (sanitised).
     */
    function _codeAmount(code, text) {
      // Primary: standard format with readable label between code and amount
      const re = new RegExp(
        //  code word-boundary
        "\\b" + code + "\\b" +
        //  label body: letters / spaces / slashes / parens — no digits allowed
        //  (one or more chars, non-greedy stop before the amount)
        "[A-Za-z\\s\\/\\(\\)]+" +
        //  optional parenthetical code echo e.g. "(64)"
        "(?:\\(\\d+\\))?" +
        //  optional whitespace or colon separator
        "[\\s:]*" +
        //  CAPTURE: the amount (digits with optional commas)
        "([\\d,]+)",
        "i"
      );
      ...
      // Fallback: pathological merge where label and amount are fused.
      // Look for the code at a word boundary followed immediately (≤ 2 chars
      // gap) by a digit run of ≥ 3 digits — avoids matching Sl.No sequences.
      const reFused = new RegExp(
        "\\b" + code + "\\b.{0,2}([1-9][\\d,]{2,})",
        "i"
      );
      const m2 = text.match(reFused);
      return m2 ? parseAmount(m2[1]) : 0;
    }

    // =========================================================================
    // LAYER C — EXTRACT EACH ALLOWANCE BY ITS STATUTORY CODE
    //
    // Components that may legitimately be 0 (e.g. PGA, Rural) will return 0
    // when their code line is absent — that is correct behaviour, not a bug.
    // =========================================================================

    result.da             = _codeAmount(22, fullText);   // DA / ADA
    result.hra            = _codeAmount(23, fullText);   // HRA
    result.cca            = _codeAmount(24, fullText);   // CCA
    result.pgAllowance    = _codeAmount(64, fullText);   // PG Allowance
    result.ruralAllowance = _codeAmount(45, fullText);   // Rural Allowance

    // Pay — Code 01 is the primary source.
    result.pay = _codeAmount("01", fullText);

    // =========================================================================
    // LAYER D — PAY FALLBACK: TABLE ROW SCAN
    //
    // Triggered only when Code 01 returned 0 (arrear bills or unusual formats
    // where the Pay line omits the code prefix entirely).
    //
    // Strategy:
    //   1. Flatten newlines so the entire Total row is a single searchable line.
    //   2. Find every sequence of ≥ 6 whitespace-separated numbers (a table row).
    //   3. Among those rows, find the one whose numbers include grossAmount.
    //   4. In that row, index 0 is the B Pay/L.Sal sentinel — skip it.
    //      Index 1 is Basic Less OA/SA → Pay.
    //
    // We only enter this branch when pay === 0 AND grossAmount > 0, so arrear
    // bills that genuinely have zero pay are not incorrectly patched.
    // =========================================================================

    if (result.pay === 0 && result.grossAmount > 0) {
      // Flatten to single line for row-level scanning
      const flat = fullText.replace(/\n/g, " ");

      // A "table row" is a run of 6+ space-separated integers/comma-numbers
      const rowRe = /\b((?:[\d,]+\s+){5,}[\d,]+)\b/g;
      let rowMatch;

      while ((rowMatch = rowRe.exec(flat)) !== null) {
        const tokens = rowMatch[1].trim().split(/\s+/).map(t => parseAmount(t));

        // Confirm this row contains the known gross amount (within ±1 for
        // floating-point rounding safety)
        const containsGross = tokens.some(
          t => Math.abs(t - result.grossAmount) <= 1
        );

        if (containsGross) {
          // tokens[0] = B Pay/L.Sal sentinel → skip
          // tokens[1] = Basic Less OA/SA     → Pay
          if (tokens.length >= 2 && tokens[1] >= 0) {
            result.pay = tokens[1];
          }
          break; // stop at first matching row
        }
      }
    }

    // =========================================================================
    // LAYER E — OTHER ALLOWANCE (Mathematical cross-check)
    //
    // Captures every allowance not explicitly listed by code (Special Pay,
    // Ex-Gratia, Incentive, etc.) without needing their individual codes.
    //
    // otherAllowance = Gross − (Pay + DA + HRA + CCA + PGA + Rural)
    //
    // Floored at 0: a negative result means a rounding artefact or an edge
    // case where Abstract totals slightly disagree — treat as no residual.
    // =========================================================================

    const knownSum = result.pay        +
                     result.da         +
                     result.hra        +
                     result.cca        +
                     result.pgAllowance +
                     result.ruralAllowance;

    result.otherAllowance = Math.max(0, result.grossAmount - knownSum);
