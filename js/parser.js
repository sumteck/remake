...
// =========================================================================
  ...
  // STAGE 7 — BIMS PARSER (UPDATED FOR CORRECT MAPPING & OFFICE EXTRACTION)
  // =========================================================================

  function _extractBimsDetails(text) {
    const result = { brn: "", treasury: "", department: "", office: "", ddoCode: "" };

    // 1. ആദ്യം BRN (Spark Code/BRN) കണ്ടെത്തുന്നു
    const brnMatch = text.match(/(\d{15,25})\s*Period\s*of\s*claim/i) || text.match(/(?<!\d)(\d{20})(?!\d)/);
    if (brnMatch) {
      result.brn = brnMatch[1].trim();
    }

    // 2. BiMS ലോഗിക്: BRN-ന് തൊട്ടുമുൻപുള്ള കോളനുകൾ (:) ഓർഡർ പ്രകാരം സ്കാൻ ചെയ്ത് കൃത്യമായ ഡാറ്റ എടുക്കുന്നു
    if (result.brn) {
      // ഓർഡർ: : Date : Nature of Claim : Treasury : Department : DDO Code : Office \n BRN
      const blockRegex = new RegExp(`:\\s*([^:]+?)\\s*:\\s*([^:]+?)\\s*:\\s*([^:]+?)\\s*:\\s*([^:]+?)\\s*:\\s*(\\d{10})\\s*:\\s*([^:]+?)\\s*${result.brn}`, 'i');
      const match = text.match(blockRegex);
      
      if (match) {
        result.treasury   = match[3].replace(/\s+/g, " ").trim();  // ഉദാ: Sub Treasury, Perinthalmanna
        result.department = match[4].replace(/\s+/g, " ").trim();  // ഉദാ: Indian Systems of Medicines
        result.ddoCode    = match[5].trim();                       // ഉദാ: 1503320105
        result.office     = match[6].replace(/\s+/g, " ").trim();  // ഉദാ: GOVT AYURVEDA DISPENSARY; ANGADIPPURAM
        return result;
      }
    }

    // അഥവാ മുകളിലത്തെ സ്കാനിംഗ് നടന്നില്ലെങ്കിൽ ഉള്ള ബാക്കപ്പ്
    const ddoMatch = text.match(/:\s*([0-9]{10})\b/);
  ...
  async function parseBimsBillFull(file) {
    ...
    return {
      billType:       "BiMS",
      // താങ്കൾ ആവശ്യപ്പെട്ടതുപോലെ നമ്പറുകൾ പരസ്പരം മാറ്റിയിരിക്കുന്നു
      billNo:         bimsData.ddoCode,   // Bill No കോളത്തിൽ DDO Code കാണിക്കും
      treasury:       bimsData.treasury,  // Treasury പേര് കൃത്യമായി വരും
      sparkCode:      bimsData.brn,       // Spark Code/BRN കോളത്തിൽ BRN നമ്പർ വരും
      department:     deptDisplay,        // ഡിപ്പാർട്ട്മെന്റും ഓഫീസും ചേർന്ന് വരും
