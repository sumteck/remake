...
const TbrParser = (() => {
 ...
 function _extractSparkCodeString(text) {
  ...
  function _extractSparkAllowances(text) {
    ...
    // പരമാവധി 30 അക്ഷരങ്ങൾ മാത്രം സ്കിപ്പ് ചെയ്യാൻ [^\d]{0,30} ഉപയോഗിക്കുന്നു
    const bMatch = text.match(/\b(?:01|140)\s+Pay[^\d]{0,30}([0-9,]+(?:\.\d{1,2})?)/i);
    if (bMatch) basicLess = parseAmount(bMatch[1]);
    
    const dMatch = text.match(/\b(?:22|141)\s+DA[^\d]{0,30}([0-9,]+(?:\.\d{1,2})?)/i);
    if (dMatch) da = parseAmount(dMatch[1]);
    
    const hraMatch = text.match(/\b(?:123\s+)?HRA[^\d]{0,30}([0-9,]+(?:\.\d{1,2})?)/i);
    if (hraMatch) hra = parseAmount(hraMatch[1]);
    
    const ccaMatch = text.match(/\b(?:24\s+)?CCA[^\d]{0,30}([0-9,]+(?:\.\d{1,2})?)/i);
