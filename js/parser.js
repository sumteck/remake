...
function _extractBimsDetails(text) {
  ...
  // 2. Treasury — anchor right boundary on next label keyword
  //    [\s\S]+? is used instead of .+? throughout so newlines don't break matching
  const treasuryMatch = text.match(/Name of Treasury\s*:\s*([\s\S]+?)\s*Name of Department/i);
  if (treasuryMatch) result.treasury = treasuryMatch[1].trim();

  // 3. Department — anchor right boundary on next label keyword
  const deptMatch = text.match(/Name of Department\s*:\s*([\s\S]+?)\s*DDO Code/i);
  if (deptMatch) result.department = deptMatch[1].trim();

  // 4. DDO Code — grab digits immediately after "DDO Code :"
  const ddoMatch = text.match(/\bDDO Code\s*:\s*(\d+)/);
  if (ddoMatch) result.ddoCode = ddoMatch[1];

  // 5. Office name — requires two-part extraction due to a PDF.js column-order quirk:
  //
  //    The BiMS form has a two-column row. PDF.js reads left-column text first,
  //    so the flat output is:
  //      "...Medical Officer : GOVT AYURVEDA Name of Office TAN/GIR : CHNG06201G DISPENSARY;ANGADIPPURAM 0 Bill Reference..."
  //       ↑ DDO title (skip)  ↑ PART 1 ↑  ↑ label (skip) ↑              ↑ TAN code (skip)  ↑ PART 2 ↑
  //
  //    Part 1 is between "Designation of DDO : <title> :" and "Name of Office"
  //    Part 2 is between the TAN/GIR code and "Bill Reference"
  //
  //    FIX: use [\s\S]+? (not .+?) everywhere so newlines don't silently break the match.
  //    FIX: strip the DDO title prefix from Part 1 with a second regex.
  //    FIX: use \d*\s* before "Bill Reference" to absorb the stray watermark digit ("0").

  const officePart1Match = text.match(
    /Designation of DDO\s*:\s*[\s\S]*?:\s*([\s\S]+?)\s*Name of Office/i
  );
  const officePart2Match = text.match(
    /TAN\/GIR\s*:\s*\S+\s+([\s\S]+?)\s*\d*\s*Bill Reference/i
  );

  // Part 1 raw may be "Medical Officer : GOVT AYURVEDA" — strip everything up to last ":"
  const rawPart1 = officePart1Match ? officePart1Match[1].trim() : "";
  const part1 = rawPart1.replace(/^[\s\S]*:\s*/, "").trim(); // strip "Medical Officer : "
  const part2 = officePart2Match ? officePart2Match[1].trim() : "";

  result.office = part2 ? `${part1} ${part2}`.trim() : part1;

  return result;
}

// Your concatenation — now works correctly:
// deptDisplay → "Indian Systems of Medicines - GOVT AYURVEDA DISPENSARY;ANGADIPPURAM"
let deptDisplay = bimsData.department;
if (bimsData.office) {
  deptDisplay = deptDisplay
    ? `${deptDisplay} - ${bimsData.office}`
    : bimsData.office;
}
  // =========================================================================
