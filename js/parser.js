...
const TbrParser = (() => {
  ...
  function _extractTreasury(text) {
    // Name Of Treasury അല്ലെങ്കിൽ Treasury എന്നതിന് ശേഷമുള്ള എല്ലാം എടുക്കുന്നു
    const m = text.match(/(?:Name\s*Of\s*Treasury|Treasury)[\s:\-]*([^\n\r]+)/i);
    
    if (m) {
      let tName = m[1];
      
      // ഒരേ വരിയിൽ വരാൻ സാധ്യതയുള്ള മറ്റ് ലേബലുകൾ വെച്ച് കട്ട് ചെയ്യുന്നു
      tName = tName.split(/(?:Computer|Token|Scroll|Dept|DDO|Date|Form|Vide|Officer|Bill No)/i)[0];
      
      // അനാവശ്യമായ ഫോം വരകളും (___) കുത്തുകളും (....) മായ്ച്ചു കളയുന്നു
      tName = tName.replace(/[_.]{2,}/g, " ").trim();
      
      // അവസാനമായി എന്തെങ്കിലും ചിഹ്നങ്ങൾ ബാക്കി വന്നാൽ അത് റിമൂവ് ചെയ്യുന്നു
