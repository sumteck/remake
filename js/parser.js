...
function _extractSparkCode(text) {
 ...
 function _extractRemarks(text) {
    ...
    // 1. ഫസ്റ്റ് ചോയ്സ്: "PAY AND ALLOWANCE IN RESPECT OF" കഴിഞ്ഞ് വരുന്ന മാസം വരെയുള്ള ഭാഗം കൃത്യമായി മുറിച്ചെടുക്കുന്നു
    ...
    if (match) {
      // അധികമുള്ള സ്പേസുകൾ കളഞ്ഞ് നീളമുള്ള പേര് കൃത്യമായി തിരികെ നൽകുന്നു (ഉദാ: Establishment FOR March 2026)
      return match[1].trim().replace(/\s+/g, " ");
    }

    // 2. ബാക്കപ്പ്: അഥവാ മുകളിലത്തെ വരി ഇല്ലെങ്കിൽ പഴയതുപോലെ "FOR Month Year" എടുക്കുന്നു
...
// =========================================================================
  ...
  // =========================================================================
  // STAGE 7 — BIMS PARSER (PERFECTED WITH COLON SEQUENCE MATCHING)
  // =========================================================================

 function _extractBimsDetails(text) {
  const result = { brn: "", treasury: "", department: "", office: "", ddoCode: "" };

  // PDF.js produces flat text. The real structure for this BiMS form is:
  //
  //   "...Name of Treasury : Sub Treasury, Perinthalmanna Name of Department :..."
  //   "...Name of Department : Indian Systems of Medicines DDO Code :..."
  //   "...DDO Code : 1503320105 Designation of DDO : Medical Officer : GOVT AYURVEDA Name of Office TAN/GIR : CHNG06201G DISPENSARY;ANGADIPPURAM 0 Bill Reference..."
  //
  // KEY QUIRK: "Name of Office" label appears AFTER the first half of the office value
  // in the flat text, because PDF.js reads left column first (value), then right column
  // label. The office name is split: Part1 is before "Name of Office", Part2 is after
  // the TAN/GIR code and before "Bill Reference Number".

  // 1. BRN — exactly 20 consecutive digits
  const brnMatch = text.match(/(?<!\d)(\d{20})(?!\d)/);
  if (brnMatch) result.brn = brnMatch[1];

  // 2. Treasury — anchor RIGHT boundary on the next label keyword
  const treasuryMatch = text.match(/Name of Treasury\s*:\s*(.+?)\s*Name of Department/);
  if (treasuryMatch) result.treasury = treasuryMatch[1].trim();

  // 3. Department — anchor RIGHT boundary on the next label keyword
  const deptMatch = text.match(/Name of Department\s*:\s*(.+?)\s*DDO Code/);
  if (deptMatch) result.department = deptMatch[1].trim();

  // 4. DDO Code — grab the digit sequence immediately after "DDO Code :"
  //    Stop at first non-digit to avoid bleeding into "Designation of DDO"
  const ddoMatch = text.match(/\bDDO Code\s*:\s*(\d+)/);
  if (ddoMatch) result.ddoCode = ddoMatch[1];

  // 5. Office — two-part extraction due to the PDF.js column-ordering quirk:
  //    Part 1: text between "Designation of DDO : <title> :" and "Name of Office"
  //    Part 2: text between TAN/GIR value and "Bill Reference Number"
  const officePart1Match = text.match(
    /Designation of DDO\s*:.*?:\s*(.+?)\s*Name of Office/
  );
  const officePart2Match = text.match(
    /TAN\/GIR\s*:\s*\S+\s+(.+?)\s+(?:\d+\s+)?Bill Reference/
  );
  const part1 = officePart1Match ? officePart1Match[1].trim() : "";
  const part2 = officePart2Match ? officePart2Match[1].trim() : "";
  result.office = part2 ? `${part1} ${part2}` : part1;
  ...
  async function parseBimsBillFull(file) {
    ...
    let deptDisplay = bimsData.department;
    if (bimsData.office) {
      deptDisplay = deptDisplay ? (deptDisplay + " - " + bimsData.office) : bimsData.office;
    }

    return {
      billType:       "BiMS",
      billNo:         "",                 // Bill No കോളം വെറുതെ വിടുന്നു
      treasury:       bimsData.treasury,  // Treasury പേര് കൃത്യമായി വരും
      sparkCode:      bimsData.brn,       // Spark Code/BRN കോളത്തിൽ BRN വരും
      department:     deptDisplay,        // Department-ഉം Office-ഉം ഒന്നിച്ച് വരും
