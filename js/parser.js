...
// =========================================================================
  ...
  // =========================================================================
  // STAGE 7 — BIMS PARSER (FIXED WITH EXACT LINE MATCHING)
  // =========================================================================

  function _extractBimsDetails(text) {
    const result = { brn: "", treasury: "", department: "", office: "", ddoCode: "" };

    // 1. BRN (Bill Reference Number) 20 അക്കങ്ങൾ
    const brnMatch = text.match(/(?<!\d)(\d{20})(?!\d)/);
    if (brnMatch) result.brn = brnMatch[1].trim();

    // 2. Treasury 
    const trMatch = text.match(/Name\s+of\s+Treasury\s*[:\-]\s*([^\n]+)/i);
    if (trMatch) result.treasury = trMatch[1].trim();

    // 3. Department 
    const deptMatch = text.match(/Name\s+of\s+Department\s*[:\-]\s*([^\n]+)/i);
    if (deptMatch) result.department = deptMatch[1].trim();

    // 4. DDO Code
    const ddoMatch = text.match(/DDO\s+Code\s*[:\-]\s*(\d{10})/i);
    if (ddoMatch) result.ddoCode = ddoMatch[1].trim();

    // 5. Office (പേര് രണ്ട് വരിയിലായി വന്നാലും കൃത്യമായി എടുക്കാൻ)
    const offMatch = text.match(/Name\s+of\s+Office\s*[:\-]\s*(.*?)(?:\s+TAN\/GIR|$)/i);
    if (offMatch) {
      let officeName = offMatch[1].trim();
      // TAN/GIR ന്റെ താഴെയുള്ള വരി കൂടി (Bill Reference ന് മുകളിൽ ഉള്ളത്) എടുക്കുന്നു
      const extraMatch = text.match(/TAN\/GIR\s*[:\-]\s*[^\n]+\n([\s\S]*?)(?=\bBill\s+Reference|\b\d{15,25}\b)/i);
      if (extraMatch) {
        officeName += " " + extraMatch[1].replace(/\n/g, " ").trim();
      }
      result.office = officeName.trim();
    }
  ...
  async function parseBimsBillFull(file) {
    ...
    return {
      ...
      treasury:       bimsData.treasury,  // Treasury പേര് വരും
