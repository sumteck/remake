/**
 * api.js
 * Bulletproof Google Sheets API interactions (Always forces Header to Row 1)
 */
const TbrApi = (() => {
  ...
  async function _request(url, options = {}, retries = 3) {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      ...
      throw new Error(`Sheets Error: ${err?.error?.message || resp.statusText}`);
    }
  }
  ...
  async function savePeriodData(finYear, month, dataRows) {
    ...
    // 1. നിർബന്ധമായും ആദ്യത്തെ വരിയിൽ ഹെഡിങ് എഴുതിച്ചേർക്കുന്നു (ഇത് ഗൂഗിൾ ഷീറ്റ് എറർ ഒഴിവാക്കും)
    ...
    // 2. ആ മാസത്തെ പഴയ ഡാറ്റകൾ ഷീറ്റിൽ ഉണ്ടെങ്കിൽ അത് ഡിലീറ്റ് ചെയ്യുന്നു
    await _deleteRowsForPeriod(id, sheetName, finYear, month);

    if (!dataRows || dataRows.length === 0) return;

    // 3. പുതിയ ഡാറ്റ രണ്ടാമത്തെ വരി മുതൽ സേവ് ചെയ്യുന്നു
    const valuesToAppend = dataRows.map(row => [finYear, month, ...row]);
